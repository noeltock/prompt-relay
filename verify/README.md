...
bash verify/wait-on-liveness.sh --id <job-id> --budget 480 --stall 600
```

Polls the companion's per-job `updatedAt`, which advances on each turn, instead of imposing a
deadline. Exits **0** finished, **2** still working so call again, **3** stall threshold reached (`updatedAt` frozen
...
Budget expiry also exits **1** if the latest fetch was unavailable; it cannot report "running".
...
This avoids treating budget expiry as a failed job. A stalled timestamp remains a heuristic,
not proof that a worker is hung.
